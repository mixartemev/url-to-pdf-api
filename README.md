[![Deploy](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/mixartemev/url-to-pdf-api)

[![Build Status](https://travis-ci.com/mixartemev/url-to-pdf-api.svg?branch=master)](https://travis-ci.org/mixartemev/url-to-pdf-api)
...
It's fairly easy to expose the contents of files on the server. You have been warned!. See https://github.com/mixartemev/url-to-pdf-api/issues/12 for background.*
...
Usage is as simple as https://pdfer2.herokuapp.com/?url=http://google.com. There's also a `POST /` if you prefer to send options in the body.
...
https://pdfer2.herokuapp.com/?url=http://google.com

**The most minimal example, render google.com as PNG image**

https://pdfer2.herokuapp.com/?output=screenshot&url=http://google.com


**Use the default @media print instead of @media screen.**

https://pdfer2.herokuapp.com/?url=http://google.com&emulateScreenMedia=false

**Use scrollPage=true which tries to reveal all lazy loaded elements. Not perfect but better than without.**

https://pdfer2.herokuapp.com/?url=http://www.andreaverlicchi.eu/lazyload/demos/lazily_load_lazyLoad.html&scrollPage=true

**Render only the first page.**

https://pdfer2.herokuapp.com/?url=https://en.wikipedia.org/wiki/Portable_Document_Format&pdf.pageRanges=1

**Render A5-sized PDF in landscape.**

https://pdfer2.herokuapp.com/?url=http://google.com&pdf.format=A5&pdf.landscape=true

**Add 2cm margins to the PDF.**

https://pdfer2.herokuapp.com/?url=http://google.com&pdf.margin.top=2cm&pdf.margin.right=2cm&pdf.margin.bottom=2cm&pdf.margin.left=2cm

**Wait for extra 1000ms before render.**

https://pdfer2.herokuapp.com/?url=http://google.com&waitFor=1000

**Wait for an element macthing the selector `input` appears.**

https://pdfer2.herokuapp.com/?url=http://google.com&waitFor=input

**Render HTML sent in JSON body**

```bash
curl -o html.pdf -XPOST -d'{"html": "<body>test</body>"}' -H"content-type: application/json" https://pdfer2.herokuapp.com/
```

**Render HTML sent as text body**

```bash
curl -o html.pdf -XPOST -d@page.html -H"content-type: text/html" https://pdfer2.herokuapp.com/
```
...
is internally used by this API. The [render code](https://github.com/mixartemev/url-to-pdf-api/blob/master/src/core/pdf-core.js#L26)
...
pdf.headerTemplate | string | - | HTML template to use as the header of each page in the PDF. **Currently Puppeteer basically only supports a single line of text and you must use pdf.margins+CSS to make the header appear!** See https://github.com/mixartemev/url-to-pdf-api/issues/77.
pdf.footerTemplate | string | - | HTML template to use as the footer of each page in the PDF. **Currently Puppeteer basically only supports a single line of text and you must use pdf.margins+CSS to make the footer appear!** See https://github.com/mixartemev/url-to-pdf-api/issues/77.
...
```bash
curl -o google.pdf https://pdfer2.herokuapp.com/?url=http://google.com
```
...
```bash
curl -o google.pdf -XPOST -d'{"url": "http://google.com"}' -H"content-type: application/json" https://pdfer2.herokuapp.com/
```
...
Supports exactly the same query parameters as `GET /`, except `url`
paremeter.
...
curl -o html.pdf -XPOST -d@receipt.html -H"content-type: text/html" https://pdfer2.herokuapp.com/?pdf.scale=1
```
